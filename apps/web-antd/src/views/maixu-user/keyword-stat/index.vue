...
async function loadBoundRooms() {
  ...
  try {
    ...
    formState.tag_room = queryState.roomWxid;
...
async function submitForm() {
  ...
  try {
    ...
    await appendOperationLog(
      '麦序机器人-用户/关键字统计',
      editingRecord.value ? '修改关键字统计' : '新增关键字统计',
      {
        tag_room: formState.tag_room,
        tag_type: formState.tag_type,
        tag_one: formState.tag_one,
      },
    );
...
async function onDelete(row: KeywordStatRecord) {
  ...
  try {
    ...
    await appendOperationLog('麦序机器人-用户/关键字统计', '删除关键字统计', {
      id: row.id,
      tag_room: row.tag_room,
...
<template>
  <Page title="关键字统计" description="自动加载已绑定群聊，支持多条件筛选、增删改查、导出Excel。">
    <Card class="mb-4">
    ...
    <Modal v-model:open="formModalOpen" :confirm-loading="formSaving" :ok-button-props="{ disabled: !canEditKeyword }" :title="editingRecord ? '修改关键字统计' : '新增关键字统计'" width="700px" @ok="submitForm">
      <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
        <Select
          v-model:value="formState.tag_room"
          :options="roomOptions"
          placeholder="群ID"
          show-search
          :filter-option="(input, option) => String(option?.label || '').toLowerCase().includes(input.toLowerCase())"
